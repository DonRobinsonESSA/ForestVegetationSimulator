...
C     LOCAL VARIABLES
     ...
     >        fsql3_step,fsql3_reset,fsql3_bind_text
      ...
      CHARACTER*8      TID,CSP1,CSP2,CSP3
      CHARACTER*1000   SQLStmtStr
...
C     LOCAL VARIABLES
      ...
      CHARACTER*1000    SQLStmtStr
      INTEGER           ColNumber,I,IHT,iRet
      DOUBLE PRECISION  LIGHTB(MXHT)
...
C     PRINT THE HEADER INFORMATION FOR THE TREELIST CYCLE, YEAR, LIGHT.
     ...
     -      'Light double null);'//CHAR(0)
      ...
      WRITE(SQLStmtStr,*) 'INSERT INTO FVS_DM_Light ',
     -  '(CaseID,StandID,Year,',
     -  'Ht,Light) VALUES (''',CASEID,''',''',TRIM(NPLT),''',?,?,?);'
...
C     CALCULATE CUMULATIVE LIGHT (AS IN **DMFSHD**) IN DOUBLE
C     FROM THE TOP OF THE CANOPY DOWNWARD

      LIGHTB(MXHT) = 1.0 - SHADE(MXHT)
      DO I = MXHT - 1, 1, -1
        LIGHTB(I) = LIGHTB(I + 1) * (1.0 - SHADE(I))
      ENDDO
...
C       BIND SQL STATEMENT PARAMETERS TO FORTRAN VARIABLES
        ...
        ColNumber=ColNumber+1       ! 2 HEIGHT (METERS)
        iRet=fsql3_bind_int(IoutDBref,ColNumber,IHT)

        ColNumber=ColNumber+1       ! LIGHT percentage at the bottom of the grid cell
        iRet=fsql3_bind_double(IoutDBref,ColNumber,LIGHTB(I))
